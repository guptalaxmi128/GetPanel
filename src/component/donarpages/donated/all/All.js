...
const All = () => {
  ...
  const { data, isSuccess } = useGetDonationDonatedAllQuery();
  // console.log(data);
  ...
  useEffect(() => {
    if (downloadReceipt && downloadReceipt.data && downloadReceiptIsSuccess) {
    ...
    <>
      <div className=" space-y-5">
        ...
        bg-warning-500"
                              ...
                              <td className="table-td ">
                                <Link
                                  to={`student/profile/${entry.studentUID}`}
                                >
      ...
      {showDataReceiver && downloadReceiptIsSuccess && (
        <RegisterDonarReceipt download={receipt} />
      )}
      <ToastContainer />
    </>
  );
